...
/* modified for simplgov by mythos234 (2015) */
...
#include <trace/events/cpufreq_simplgov.h>

struct cpufreq_simplgov_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_simplgov_cpuinfo, cpuinfo);
...
struct cpufreq_simplgov_tunables {
	int usage_count;
...
static struct cpufreq_simplgov_tunables *common_tunables;
static struct cpufreq_simplgov_tunables *tuned_parameters[NR_CPUS] = {NULL, };
...
static void cpufreq_simplgov_timer_resched(
	struct cpufreq_simplgov_cpuinfo *pcpu)
{
	struct cpufreq_simplgov_tunables *tunables =
 ...
 * function.
 */
static void cpufreq_simplgov_timer_start(
	struct cpufreq_simplgov_tunables *tunables, int cpu)
{
	struct cpufreq_simplgov_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int freq_to_targetload(
	struct cpufreq_simplgov_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_simplgov_cpuinfo *pcpu,
		unsigned int loadadjfreq)
{
	

	unsigned int freq = pcpu->policy->cur;
	unsigned int simpltimer;
...
static unsigned int check_mode(int cpu, unsigned int cur_mode, u64 now)
{
	...
	trace_cpufreq_simplgov_mode(cpu, total_load,
...
static void enter_mode(struct cpufreq_simplgov_tunables * tunables)
{
...
static void exit_mode(struct cpufreq_simplgov_tunables * tunables)
{
...
static void cpufreq_simplgov_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	...
	if (pcpu->target_freq == new_freq &&
		...
		trace_cpufreq_simplgov_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
	...
	trace_cpufreq_simplgov_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_simplgov_timer_resched(pcpu);
...
static void cpufreq_simplgov_idle_start(void)
{
	struct cpufreq_simplgov_cpuinfo *pcpu =
...
static void cpufreq_simplgov_idle_end(void)
{
	struct cpufreq_simplgov_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_simplgov_timer(smp_processor_id());
	}
...
#ifdef CONFIG_PMU_COREMEM_RATIO
static void region_update_status(struct cpufreq_simplgov_tunables *tunables)
{
...
static int cpufreq_simplgov_regionchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_simplgov_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
...
static int cpufreq_simplgov_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	cpumask_t policy_mask;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			trace_cpufreq_simplgov_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static void cpufreq_simplgov_boost(const struct cpufreq_policy *policy)
{
	int i;
	int anyboost = 0;
...
static int cpufreq_simplgov_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_simplgov_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_simplgov_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_simplgov_tunables *tunables,
		char *buf)
{
#ifdef CONFIG_MODE_AUTO_CHANGE
...
static ssize_t store_hispeed_freq(struct cpufreq_simplgov_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
#ifdef CONFIG_MODE_AUTO_CHANGE
...
static ssize_t store_go_hispeed_load(struct cpufreq_simplgov_tunables
...
static ssize_t show_min_sample_time(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
#ifdef CONFIG_MODE_AUTO_CHANGE
...
static ssize_t store_min_sample_time(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_simplgov_tunables *tunables,
		char *buf)
{
#ifdef CONFIG_MODE_AUTO_CHANGE
...
static ssize_t store_timer_rate(struct cpufreq_simplgov_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_slack(struct cpufreq_simplgov_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_simplgov_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_simplgov_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_simplgov_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_simplgov_boost("on");
		cpufreq_simplgov_boost(policy);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_simplgov_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_simplgov_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_simplgov_boost("pulse");
	cpufreq_simplgov_boost(policy);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_simplgov_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_simplgov_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
#ifdef CONFIG_MODE_AUTO_CHANGE
static ssize_t show_mode(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->mode);
}

static ssize_t store_mode(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_enforced_mode(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->enforced_mode);
}

static ssize_t store_enforced_mode(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_param_index(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->param_index);
}

static ssize_t store_param_index(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_multi_enter_load(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->multi_enter_load);
}

static ssize_t store_multi_enter_load(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_multi_exit_load(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->multi_exit_load);
}

static ssize_t store_multi_exit_load(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_single_enter_load(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->single_enter_load);
}

static ssize_t store_single_enter_load(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_single_exit_load(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->single_exit_load);
}

static ssize_t store_single_exit_load(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_multi_enter_time(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->multi_enter_time);
}

static ssize_t store_multi_enter_time(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_multi_exit_time(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->multi_exit_time);
}

static ssize_t store_multi_exit_time(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_single_enter_time(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->single_enter_time);
}

static ssize_t store_single_enter_time(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_single_exit_time(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->single_exit_time);
}

static ssize_t store_single_exit_time(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_cpu_util(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	int i;
	u64 now;
...
static ssize_t show_single_cluster0_min_freq(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->single_cluster0_min_freq);
}

static ssize_t store_single_cluster0_min_freq(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_multi_cluster0_min_freq(struct cpufreq_simplgov_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->multi_cluster0_min_freq);
}

static ssize_t store_multi_cluster0_min_freq(struct cpufreq_simplgov_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
#endif
#ifdef CONFIG_PMU_COREMEM_RATIO
static ssize_t show_region_time_in_state(struct cpufreq_simplgov_tunables *tunables,
			  char *buf)
{
	int i;
	ssize_t len = 0;
...
static struct attribute *simplgov_attributes_gov_sys[] = {
...
static struct attribute_group simplgov_attr_group_gov_sys = {
	.attrs = simplgov_attributes_gov_sys,
	.name = "simplgov",
};

/* Per policy governor instance */
static struct attribute *simplgov_attributes_gov_pol[] = {
...
static struct attribute_group simplgov_attr_group_gov_pol = {
	.attrs = simplgov_attributes_gov_pol,
	.name = "simplgov",
};

#ifdef CONFIG_ANDROID
static const char *simplgov_sysfs[] = {
	"target_loads",
	"above_hispeed_delay",
	"hispeed_freq",
	"go_hispeed_load",
...
{
	if (have_governor_per_policy())
		return &simplgov_attr_group_gov_pol;
	else
		return &simplgov_attr_group_gov_sys;
}
...
static int cpufreq_simplgov_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
#ifdef CONFIG_PMU_COREMEM_RATIO
	...
	switch (val) {
	case IDLE_START:
		cpufreq_simplgov_idle_start();
#ifdef CONFIG_PMU_COREMEM_RATIO
		pmu_suspend_cpu(cpu);
#endif
		break;
	case IDLE_END:
		cpufreq_simplgov_idle_end();
#ifdef CONFIG_PMU_COREMEM_RATIO
		pmu_resume_cpu(cpu);
#endif
		break;
...
static struct notifier_block cpufreq_simplgov_idle_nb = {
	.notifier_call = cpufreq_simplgov_idle_notifier,
};
...
static void change_sysfs_owner(struct cpufreq_policy *policy)
{
	...
	for (i = 0; i < ARRAY_SIZE(simplgov_sysfs); i++) {
		snprintf(buf, sizeof(buf), "/sys%s/simplgov/%s", path,
				simplgov_sysfs[i]);
...
#ifdef CONFIG_MODE_AUTO_CHANGE
static void cpufreq_param_set_init(struct cpufreq_simplgov_tunables *tunables)
{
	unsigned int i;
...
static int cpufreq_governor_simplgov(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!policy->governor->initialized) {
			idle_notifier_register(&cpufreq_simplgov_idle_nb);
#ifdef CONFIG_PMU_COREMEM_RATIO
...
#endif
				idle_notifier_unregister(&cpufreq_simplgov_idle_nb);
			}
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_simplgov_timer_start(tunables, j);
			pcpu->governor_enabled = 1;
			up_write(&pcpu->enable_sem);
		}

		snprintf(speedchange_task_name, TASK_NAME_LEN, "cfsimplgov%d\n",
					policy->cpu);

		tunables->speedchange_task =
			kthread_create(cpufreq_simplgov_speedchange_task, NULL,
		...
		tunables->regionchange_task =
			kthread_create(cpufreq_simplgov_regionchange_task, NULL,
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_simplgov
static
#endif
struct cpufreq_governor cpufreq_gov_simplgov = {
	.name = "simplgov",
	.governor = cpufreq_governor_simplgov,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_simplgov_nop_timer(unsigned long data)
{
}

unsigned int cpufreq_simplgov_get_hispeed_freq(int cpu)
{
	struct cpufreq_simplgov_cpuinfo *pcpu =
			&per_cpu(cpuinfo, cpu);
	struct cpufreq_simplgov_tunables *tunables;
...
static int cpufreq_simplgov_cluster1_min_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	...
	trace_cpufreq_simplgov_cpu_min_qos(cpu, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_simplgov_cluster1_min_qos_notifier = {
	.notifier_call = cpufreq_simplgov_cluster1_min_qos_handler,
};

static int cpufreq_simplgov_cluster1_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_simplgov_cpuinfo *pcpu;
	struct cpufreq_simplgov_tunables *tunables;
	...
	trace_cpufreq_simplgov_cpu_max_qos(cpu, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_simplgov_cluster1_max_qos_notifier = {
	.notifier_call = cpufreq_simplgov_cluster1_max_qos_handler,
};

#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
static int cpufreq_simplgov_cluster0_min_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_simplgov_cpuinfo *pcpu;
	struct cpufreq_simplgov_tunables *tunables;
	...
	trace_cpufreq_simplgov_kfc_min_qos(0, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_simplgov_cluster0_min_qos_notifier = {
	.notifier_call = cpufreq_simplgov_cluster0_min_qos_handler,
};

static int cpufreq_simplgov_cluster0_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_simplgov_cpuinfo *pcpu;
	struct cpufreq_simplgov_tunables *tunables;
	...
	trace_cpufreq_simplgov_kfc_max_qos(0, val, pcpu->policy->cur);
...
static struct notifier_block cpufreq_simplgov_cluster0_max_qos_notifier = {
	.notifier_call = cpufreq_simplgov_cluster0_max_qos_handler,
};
#endif
#endif

static int __init cpufreq_simplgov_init(void)
{
	unsigned int i;
	struct cpufreq_simplgov_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_simplgov_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_simplgov_nop_timer;
...
#ifdef CONFIG_ARCH_EXYNOS
	pm_qos_add_notifier(PM_QOS_CLUSTER1_FREQ_MIN, &cpufreq_simplgov_cluster1_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER1_FREQ_MAX, &cpufreq_simplgov_cluster1_max_qos_notifier);
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MIN, &cpufreq_simplgov_cluster0_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MAX, &cpufreq_simplgov_cluster0_max_qos_notifier);
#endif
#endif
...
#endif
	return cpufreq_register_governor(&cpufreq_gov_simplgov);
}
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_simplgov
fs_initcall(cpufreq_simplgov_init);
#else
module_init(cpufreq_simplgov_init);
#endif

static void __exit cpufreq_simplgov_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_simplgov);
}

module_exit(cpufreq_simplgov_exit);
